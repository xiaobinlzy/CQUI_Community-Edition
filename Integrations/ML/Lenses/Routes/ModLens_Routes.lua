...
local function OnUnitSelectionChanged( playerID:number, unitID:number, hexI:number, hexJ:number, hexK:number, bSelected:boolean, bEditable:boolean )
    if playerID == Game.GetLocalPlayer() then
        ...
        if unitType then
            if bSelected then
                if (unitType == "UNIT_MILITARY_ENGINEER" or unitType == "UNIT_LEU_INVESTOR" or unitType == "UNIT_LEU_TYCOON" or unitType == "UNIT_ENGINEER_CORP") and AUTO_APPLY_ENGINEER_LENS then
                    ShowRoutesLens()
                end
            -- Deselection
            else
...
function OnUnitCaptured( currentUnitOwner, unit, owningPlayer, capturingPlayer )
    ...
    if owningPlayer == localPlayer then
        ...
        if unitType and (unitType == "UNIT_MILITARY_ENGINEER" or unitType == "UNIT_LEU_INVESTOR" or unitType == "UNIT_LEU_TYCOON" or unitType == "UNIT_ENGINEER_CORP") and AUTO_APPLY_ENGINEER_LENS then
            ClearRoutesLens()
        end
    end
end
